...
public class DiamondSilk {

    private final Main plugin;
    private ItemStack dsLaser;

    public DiamondSilk(Main plugin) {
        this.plugin = plugin;
        this.createLaser();
    }
    ...
    private void createLaser() {
        ...
        for (String realLore : this.plugin.getConfig().getStringList("Diamond_Silk_Lore")) {
            lore.add(Main.chatColor("" + realLore));
        }
        for (String enchants : this.plugin.getConfig().getStringList("Diamond_Silk_Enchants")) {
        ...
        meta.setDisplayName(Main.chatColor("" + this.plugin.getConfig().getString("Diamond_Silk_Name")));
        meta.setLore(lore);
